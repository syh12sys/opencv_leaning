...
{   
    ...
    if (bind(ev->conn, (struct sockaddr*)&(ev->nladdr), NL_ADDR_SIZE) < 0)
        //PEVENT_ERROR("init-connection-bind, code=%d", errno_t);
    {
        //int lastError = errno;
        printf("init-connection-bind, code=%d", errno);
    }
...
int GetProcessInfo(int pid, char* key,  char* info)
{
    int rtn = -1;
    ...
    if (NULL != fp)
    {
        while (fgets(buf, kBufferSize - 1, fp) != NULL)
        {
            //printf(buf);
            char tempKey[kBufferSize];
            char tempInfo[kBufferSize];
            sscanf(buf, "%s %s", tempKey, tempInfo);
            if (strcmp(tempKey, key) == 0)
            {
                memcpy(info, tempInfo, strlen(tempInfo));
                rtn = 0;
                break;
            }
        }
...
{
    const int kBufferSize = 1024;
    char processName[kBufferSize];
    memset(processName, 0, kBufferSize);
    GetProcessInfo(exec_event->process_pid, "Name:", processName);
    char ppid[kBufferSize];
    memset(ppid, 0, kBufferSize);
    GetProcessInfo(exec_event->process_pid, "PPid:", ppid);

    char parentProcessName[kBufferSize];
    memset(parentProcessName, 0, kBufferSize);
    GetProcessInfo(atoi(ppid), "Name:", parentProcessName);

    printf("EXEC\n\tpid:\t%d\n\ttgid:\t%d\n\tpname:\t%s\n\tppname:\t%s\n\tppid:\t%d\n", 
            exec_event->process_pid,
            exec_event->process_tgid,
            processName, parentProcessName, atoi(ppid));
